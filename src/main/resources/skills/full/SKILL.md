...
description: 执行完整的出差报销全流程：确认项目→获取文件→识别发票 → 生成报表 → 导出Excel与项目包
---

当用户请求完成报销全流程（如"一键报销"、"全流程报销"）时，按以下步骤执行：
...
4. 调用 `getRecognitionTaskProgress(projectName)` 轮询识别进度，间隔 30 秒，最多轮询 6 次（约 180 秒）。每次轮询后检查识别是否完成。
